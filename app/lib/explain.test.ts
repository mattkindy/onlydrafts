...
/** a player whose week is drawn the way a live week is, and his draws */
function player(
...
const added = (base: number, players: Contender[]) =>
  Array.from({ length: DRAWS }, (_, i) =>
    players.reduce((sum, his) => sum + his.week[i]!, base));

const openingOf = (
  others: number[], theirs: number[],
  against: string[] = [], alongside: string[] = [],
): Opening => ({ others, theirs, factors: factorAt, against, alongside });
...
describe("a swap pulled apart", () => {
   ...
   */
  it("prefers the lower projection when the other player shares their game", () => {
    const theirQb = player("theirs|qb", 22, 8, [["game|shared", 0.9]]);
    const opening = openingOf(flat(72), added(68, [theirQb]), ["game|shared"]);
    const inTheirGame = player("mine|shared", 15, 5, [["game|shared", 0.7]]);
    const elsewhere = player("mine|apart", 14, 5);
    const x = explainSwap(opening, inTheirGame, elsewhere);
  ...
  it("prefers the wider week as the underdog and the narrower as the favourite", () => {
    const steady = player("mine|steady", 15, 2);
    const wild = player("mine|wild", 15, 10);
    const theirs = added(0, [player("theirs|own", 20, 9)]);
    const behind = openingOf(flat(80), theirs.map((n) => n + 88));
    const ahead = openingOf(flat(80), theirs.map((n) => n + 64));
  ...
  it("gives back pieces that add up to the whole change", () => {
    const mine = player("mine|rest", 30, 7, [["game|both", 0.5]]);
    const opening = openingOf(
      added(40, [mine]),
      added(60, [player("theirs|own", 24, 9, [["game|both", 0.4]])]),
      ["game|both"], ["game|both"],
    );
    const x = explainSwap(
      opening,
      player("starter", 16, 6, [["game|both", 0.6]]),
      player("instead", 15, 9),
    );
  ...
  /**
   * A player who shares nothing with anybody leaves the projection to do it
   * all, and the two correlation pieces come out at zero rather than
   * at whatever the drawing happened to leave in them.
   */
  it("puts everything in the points when neither player shares a game", () => {
    const opening = openingOf(added(70, [player("mine|rest", 30, 8)]),
      added(85, [player("theirs|own", 22, 9)]));
    const x = explainSwap(opening, player("one", 12, 6), player("two", 15, 6));
  ...
  it("prices the slot the same way whichever player is asked about", () => {
    const starter = player("one", 12, 6);
    const instead = player("two", 15, 6);
    const opening = openingOf(added(70, [player("mine|rest", 30, 8)]),
      added(85, [player("theirs|own", 22, 9)]));
    const x = explainSwap(opening, starter, instead);

    expect(chanceWith(opening, starter)).toBeCloseTo(x.odds, 12);
    expect(chanceWith(opening, instead) - chanceWith(opening, starter))
