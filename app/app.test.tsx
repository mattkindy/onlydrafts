...
function aLeague(named = "Mildred League XIV", team = "mattkindy"): League {
  const players = file.players.filter((p) => p.position !== "DEF").slice(0, 60);
  const mine = players.slice(0, 12);
  ...
  return {
    ...
    allRosters: [
      ...
      ...others.map((who, i) => ({
        owner: who,
        picks: every,
        keys: players.slice(12 + i * 8, 20 + i * 8)
...
describe("the views", () => {
  ...
  it("draws your roster", () => {
    render(
      <Roster
        byKey={byKey} players={players} league={league} season={2026} perTeam={3}
  ...
  it("draws the draft board", async () => {
    render(
      <DraftView
        players={players} state={NO_DRAFT} teams={12} snake posFilter="ALL"
...
describe("the board in a league's terms", () => {
  it("gives the best value to whoever it has first", () => {
    const players = boardFor(aLeague())
     ...
     */
    for (let i = 1; i < Math.min(80, players.length); i++) {
      expect(players[i]!.vor!, players[i]!.name)
        .toBeLessThanOrEqual(players[i - 1]!.vor! + 0.001);
    }
  });

  it("is put back in order when another league is chosen", () => {
    const first40 = (players: Player[]) =>
      players.slice(0, 40).map((p) => p.key).join(",");
...
describe("a name somebody else chose", () => {
  ...
  it("is drawn as text, not as markup", () => {
    ...
    render(
      <Roster
        byKey={new Map(players.map((p) => [p.key, p]))} players={players}
 ...
 * were drawn from different fields: the same player read 20.1 in the table
...
describe("the rate and the season agree", () => {
  ...
  it("shows expected games next to both", async () => {
    const games = players[0]!.games!.toFixed(1);
  ...
  it("takes expected games from the simulation, not a flat season", () => {
    // every player at 17 games is what happens when nothing reads sim.games,
    // and it quietly changes every value on the board
    expect(players.some((p) => p.games !== 17)).toBe(true);
  });
});
...
describe("the line on the card adds up", () => {
  it("prints his own points under his own league's rules", async () => {
    ...
    for (const rec of [0, 0.5, 1]) {
      const players = rescore(file.players, { teams: 12, slots: SLOTS, pays: at(rec) })
  ...
  it("scores a season to the points a game beside it", async () => {
    ...
    const { payFor } = await import("./lib/scoring.ts");
    const league = aLeague();
    const players = boardFor(league).filter((p) => p.projected);
  ...
  it("shows a season as the game line times the games", async () => {
    const { lineOver } = await import("./lib/statLine.ts");
    const players = boardFor(aLeague()).filter((p) => p.projected && p.games);
    const p = players[0]!;
  ...
  it("gives each position the categories it is read by", async () => {
    ...
    const labels = (position: string) => {
      const p = players.find((m) => m.position === position && m.projected);
...
describe("a week row keeps its shape", () => {
  it("says the week, the opponent, the bar and the points", async () => {
    ...
    const players = boardFor(aLeague());
    const p = players.find((m) => (m.weeks?.length ?? 0) > 0 && m.projected)!;
...
describe("the scoring in play is on screen", () => {
  ...
  it("scores a player by what his league pays and not by a guess", async () => {
    const { payFor } = await import("./lib/scoring.ts");
  ...
  /**
   * ESPN prices a play per position where two players are paid differently
   * for it, and parks the plain price at zero when it does. Reading
...
describe("a league saved by an older build", () => {
  ...
  it("still scores a board when the league says nothing", () => {
    const players = rescore(file.players, { teams: 12, slots: SLOTS, pays: {} });

    expect(players.length).toBeGreaterThan(500);
    expect(players[0]!.ppg).toBeGreaterThan(0);
  });
});
...
describe("the draft list runs down the value", () => {
  ...
  it("never puts a bigger value below a smaller one", () => {
    ...
    const worth = new Map(players.map((p) => [p.name, p.vor ?? 0]));

    /**
     * Each list on its own. The whole board is drawn as cards under the
     * shortlist and starts again from the best player, so reading every
 ...
 * What a pick buys and what a player is worth against it come off the
...
describe("the keeper sheet works on one scale", () => {
  ...
  it("adds up on the sheet", async () => {
    ...
    const p = players.find((m) => m.projected)!;
    const costPick = pickForRound(3, draft);
    const sums = keeperSums(players, p, costPick, draft);
 ...
 * So the top card can show a number that belongs to the player below it.
...
describe("a card shows both values when they disagree", () => {
  it("keeps his own beside the board's", () => {
    ...
    // his own is worked out from his own points, so a player who scores
    // more than another at his position is worth more by it
    const backs = players.filter((p) => p.position === "RB" && p.projected)
      .slice(0, 30);
    ...
    const apart = players.slice(0, 40)
      .filter((p) => Math.abs((p.ownVor ?? 0) - (p.vor ?? 0)) >= 10);
    expect(apart.length).toBeGreaterThan(3);
  });

  it("draws the chip on a player the room and the model disagree about", () => {
    const players = boardFor(aLeague());
    const p = players.find((m) =>
      Math.abs((m.ownVor ?? 0) - (m.vor ?? 0)) >= 10 && m.projected)!;

    render(
      <DraftView players={[p, ...players.filter((m) => m !== p)]} state={NO_DRAFT}
 ...
 * conflate: one hides players you cannot start, the other reorders by what
 * a player adds to the lineup you have left.
 */
describe("drafting for what you still need", () => {
  const league = aLeague();
  const players = boardFor(league);
  const backs = players.filter((p) => p.position === "RB").slice(0, 4);
  ...
  /**
   * The shortlist only. The page draws your own drafted players further
  ...
  it("says which slots are open and what the room has taken", () => {
    render(
      <DraftView players={players} state={withMine} teams={12} snake posFilter="ALL"
  ...
  it("hides a position once you cannot start another of them", () => {
    ...
    const shown = new Set(
      namesIn(where).map((name) => players.find((p) => p.name === name)?.position),
    );
 ...
 */
describe("who the injury report has listed", () => {
  const league = aLeague();
  const players = boardFor(league);
  const hurtOne = players[2]!;
  const outOne = players[4]!;
  ...
  it("says nothing about a player with nothing wrong with him", () => {
    render(
      <DraftView players={players} state={NO_DRAFT} teams={12} snake posFilter="ALL"
        query="" order="rank" slots={league.slots} onMore={() => {}} />,
      where,
    );
...
describe("the whole board as cards", () => {
   ...
   * above this saying the same players in the same order, from when cards
   * were the only place a player was drawn properly.
   */
  ...
  it("still has the table for looking a player up", async () => {
    const at = draw();

    showTable(at);
    await Promise.resolve();

    expect(at.querySelectorAll("table.ranks tbody tr").length).toBe(players.length);
  });
});
...
describe("a slot you cannot leave empty", () => {
  ...
  const cardFor = (position: string) => {
    ...
    document.body.appendChild(at);
    render(
      <DraftView players={players} state={NO_DRAFT} teams={12} snake
...
/**
 * The board ships a few players at a tenth of a point a game, and the
 ...
 */
describe("a player too thin to have a spread", () => {
  const league = aLeague();
  const players = boardFor(league);

  it("is given no spread rather than an enormous one", () => {
    const thin = players.filter((p) => (p.ppg ?? 0) > 0 && (p.ppg ?? 0) < 3);
     ...
     * a player varies. The board drops anybody at ten or wider, so fifteen
...
/**
 * Two players on the same average are not the same bet. Brock Purdy's
 ...
 */
describe("what a player is worth, over the middle of his seasons", () => {
  const players = boardFor(aLeague());

  it("gives a band that contains its own middle", () => {
    const withBands = players.filter((p) => p.par);
  ...
  /**
   * A player who misses half a year is a wider bet than one who does not,
   * whatever the two averages say, and that is the thing the band is
   * for.
   */
  it("is wider for a player the simulation is less sure of", () => {
    const wide = players
    ...
    const spread = (its: typeof players) =>
 ...
 * weeks a player wins you while the card led with his place on our board
...
describe("the card leads with the order it is in", () => {
  ...
  it("puts the player it leads with at the top of the list", () => {
    const at = document.createElement("div");
    document.body.appendChild(at);
    render(
      <DraftView players={players} state={NO_DRAFT} teams={12} snake posFilter="ALL"
 ...
 * again underneath as keeper cards over the same twelve players. What each
...
describe("my team lists each player once", () => {
  ...
  it("leaves the keeper row off when the league has no keepers", () => {
    render(
      <Roster
        byKey={byKey} players={players} league={{ ...league, keepers: false }}
