...
describe("blendedPlace", () => {
  it("puts a player the three agree on where they all had him", () => {
   ...
   * changes is that the whole group goes behind the players the market has
   * an opinion about.
   */
  it("orders a player nobody priced by the models, then sets him back", () => {
...
describe("placesBy", () => {
  it("numbers the best player first and leaves out the ones without one", () => {
...
/**
 * Each opinion ranks only the players it can see, and it sees a different
 ...
 * covers scattered players does not: rank 250 of 440 can be the 400th player
...
describe("an opinion that speaks for only some of the board", () => {
  /** a board where every other player is missing from one opinion */
  const board = Array.from({ length: 20 }, (_, i) => ({
    key: `m${i}`,
    /** best first, so a higher number is a better player */
    worth: 20 - i,
    seen: i % 2 === 0,
  }));

  it("compresses a player's place toward the front when it skips players", () => {
    ...
    // the same player, eighth of the ten it can see
  ...
  it("is why two opinions cannot have their places added as they are", () => {
    ...
    // how far down each list he is agrees to within one player, which is
...
describe("moving an opinion onto the board's scale", () => {
  /** twenty players, and where a thing that saw all of them puts each */
  ...
  it("stretches an opinion whose players are scattered", () => {
    const everyOther = new Map(
      Array.from({ length: 10 }, (_, i) => [`m${i * 2}`, i + 1]),
    );
    const spread = spreadOver(everyOther, reference);

    // its fifth best player was at 5 of 10 and now sits where the fifth
    // of the players it can see sits on the board, which is ninth
  ...
  it("drops a player the reference has never heard of", () => {
...
describe("a player nobody has priced", () => {
  it("goes behind an equal player the market has priced", () => {
   ...
   */
  it("does not punish a player for an opinion that speaks for nobody like him", () => {
